...
(* A type of a symbol is a list of types, where head is the value type *)
...
let to_string_sproperty = function
  ...
  |DomainConstant   -> "domain constant"  (*in finted models *)
  |DomainPred       -> "domain predicate" (*in finted models *)
  |DefPred          -> "defpred"
  |Type             -> "type"
  |Connective       -> "connective"
  |Quantifier       -> "quantifier"
...
let prolog_to_stream s symb = 
  let symb_str = String.copy (to_string symb) in 
  match symb_str.[0] with 
    'a'..'z'  -> s.stream_add_str symb_str
  | 'A'..'Z'  -> 
      let symb_str_c = String.copy (to_string symb) in 
      symb_str_c.[0]<- (Char.lowercase symb_str.[0]);
      s.stream_add_str symb_str_c
  | '0'..'9'  -> 
      s.stream_add_char 'f';
